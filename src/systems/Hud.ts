      ...
      : state.paused
        ...
        : state.nearUpgrade
          ...
          : state.canUpgrade
            ? '金币够了：跟着绿色航线进金色港口'
            : '鼠标转向，左键开炮，右键换弹';
    this.upgradeLine.textContent = `靠近港口自动进入：1 火炮 Lv.${state.cannonLevel} $${state.cannonCost} · 2 船体 Lv.${state.hullLevel} $${state.hullCost} · 3 航速 Lv.${state.speedLevel} $${state.speedCost}`;
